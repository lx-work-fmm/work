...
/*DX200用定義*/
static const short	LENGTH_OF_STRING2				= 32;							//文字列変数のデータ長
static const short	LENGTH_OF_MULTI_STR2			= 14;							//文字列の複数データ最大数
...
//文字列の複数データ(DX200用)
typedef struct {
			char data[LENGTH_OF_MULTI_STR2][LENGTH_OF_STRING2+1];
}ESMultiStrData2;
...
/*DX200用コマンド*/
LONG	STDCALL	ESGetStrData2(HANDLE handle, long number, char *cp);								//文字型変数(S)の読み出し
LONG	STDCALL	ESSetStrData2(HANDLE handle, long number, char *cp);								//文字型変数(S)の書き込み
LONG	STDCALL	ESGetStrDataM2(HANDLE handle, long varNo, long number, ESMultiStrData2 *varData);	//S変数の読み出し(Multi)
LONG	STDCALL	ESSetStrDataM2(HANDLE handle, long varNo, long number, ESMultiStrData2 varData);		//S変数の書き込み(Multi)
